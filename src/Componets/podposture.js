...
import model from './Ai/mbp-v1-pipe.glb'
import img1 from './Ai/Frame 6.png'
import img2 from './Ai/Frame 8.png'
import './styles.css'
softShadows()
const rsqw = (t, delta = 0.1, a = 1, f = 1 / (2 * Math.PI)) => (a / Math.atan(1 / delta)) * Math.atan(Math.sin(2 * Math.PI * t * f) / delta)

export default function PodPosture() {
  return (
...
function Composition({ ...props }) {
  ...
  const [textureRed, textureBlue] = useTexture([img1, img2])
  ...
  return (
    <>
      ...
      <group ref={group} position={[0, -height / 2.65, 0]} {...props}>
        ...
        <M1 ref={mbp16} texture={img1} scale={width / 67}>
          <Tag ref={left} position={[16, 5, 0]} head="up to" stat="13x" expl={`faster\ngraphics\nperformance²`} />
        </M1>
        <M1 ref={mbp14} texture={img2} scale={width / 77} rotation={[0, Math.PI, 0]} position={[0, 0, -width / 2.625]}>
